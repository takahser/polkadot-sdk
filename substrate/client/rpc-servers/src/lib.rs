...
use jsonrpsee::{
	server::{
		middleware::{AuthorityError, HostFilterLayer, ProxyGetRequestLayer},
		PingConfig,
	},
	RpcModule,
};
...
fn hosts_filtering(enabled: bool, addrs: &[SocketAddr]) -> Result<HostFilterLayer, AuthorityError> {
	if enabled {
